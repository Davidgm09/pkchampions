...
export default function TypeEffectivenessTable({ types, megaName }: Props) {
  const { t } = useLanguage()
  ...
  const title = megaName
    ? t('dex.effectivityMega', { name: megaName })
    : t('dex.typeEffTitle')
      ...
      {(offensive.x4.length > 0 || offensive.x2.length > 0) && (
        <div className="border-l-2 border-champ-success/40 pl-4">
          <p className="text-xs font-bold uppercase tracking-widest mb-3 font-body text-champ-success">
            {t('dex.effectiveAgainst')}
          </p>
          <div className="flex flex-col gap-2">
            {offensive.x4.length > 0 && (
              <div className="flex flex-wrap items-center gap-1.5">
                <span className="text-xs font-mono font-bold px-1.5 py-0.5 rounded shrink-0 bg-green-500/20 text-green-300 border border-green-500/40">×4</span>
                {offensive.x4.map((tp) => <TypeBadge key={tp} type={tp} size="sm" />)}
              </div>
            )}
            {offensive.x2.length > 0 && (
              <div className="flex flex-wrap items-center gap-1.5">
                <span className="text-xs font-mono font-bold px-1.5 py-0.5 rounded shrink-0 bg-green-500/10 text-green-400/80 border border-green-500/20">×2</span>
                {offensive.x2.map((tp) => <TypeBadge key={tp} type={tp} size="sm" />)}
              </div>
            )}
          </div>
        </div>
              ...
              {section.keys.map((key) => {
                ...
                return (
                  <div key={key} className="flex flex-wrap items-center gap-1.5">
                    ...
                    {typeList.map((tp) => <TypeBadge key={tp} type={tp} size="sm" />)}
                  </div>
                )
              })}
            </div>
