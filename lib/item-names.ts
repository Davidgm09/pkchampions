...
export function getCachedItemES(enName: string): string | null {
  return esCache[enName] ?? null
}

export async function fetchItemES(enName: string): Promise<string | null> {
  if (enName in esCache) return esCache[enName]
  if (inFlight[enName]) return inFlight[enName]!
  const p = fetch(`https://pokeapi.co/api/v2/item/${toAPIName(enName)}`)
